...
contract TokenNetwork is Utils {
    ...
    struct unlockResult {
        bytes32 merkle_root;
        bytes32 unsettled_merkle_root;
        bytes32[]unsettled_merkle_layer;
        ...
        public
    {
        ...
        // Do the transfer
        require(token.transferFrom(participant, address(this), added_deposit));
    }
    ...
    function closeChannel(
        ...
        public
    {
        require(channel_identifier == getChannelIdentifier(sender, partner));
        ...
        channel.participants[sender].is_the_closer = true;
        ...
        emit ChannelClosed(channel_identifier, sender, nonce);
    }
        ...
        public
    {
        ...
        // on chain.
        unlockResult memory result;
        result = getMerkleRootAndUnlockedAmount(
            merkle_tree_leaves
        );
        ...
        require(unlock_data.locksroot == result.merkle_root);
        ...
        // the smart contract.
        result.unlocked_amount = min(result.unlocked_amount, locked_amount);

        // Transfer the rest of the tokens back to the partner
        returned_tokens = locked_amount - result.unlocked_amount- result.unsettled_amount;

        // Remove partner's unlock data
        delete unlock_identifier_to_unlock_data[unlock_key];

        if (result.unsettled_amount !=0) {
            storeUnlockData(
                channel_identifier,
                partner,
                participant,
                result.unsettled_amount,
                result.unsettled_merkle_root
            );
        }
        ...
        emit ChannelUnlocked(
            ...
            partner,
            result.merkle_root,
            result.unlocked_amount,
            result.unsettled_amount,
            result.unsettled_merkle_root,
            result.unsettled_merkle_layer,
            returned_tokens
        );

        // Transfer the unlocked tokens to the participant. unlocked_amount can
        // be 0
        if (result.unlocked_amount > 0) {
            require(token.transfer(participant, result.unlocked_amount));
        }
        ...
        assert(locked_amount >= result.unlocked_amount);
    }
...
//            uint256(MessageTypeId.BalanceProofUpdate),
    ...
    function getMerkleRootAndUnlockedAmount(bytes memory merkle_tree_leaves)
        internal
        returns (unlockResult memory)
    {
        uint256 length = merkle_tree_leaves.length;

        // each merkle_tree lock component has this form:
        // (locked_amount || expiration_block || secrethash || selected_incentive ||
        //  selected_delay || aux_incentive || aux_delay || startTime || target ) = 8 * 32 + 20 bytes
        require(length % 276 == 0);

        unlockResult memory result;
        leafData memory leaf;
        // uint256 unlocked_amount;
        // bytes32[] memory unsettled_merkle_layer;
        // uint256 unsettled_amount;
        ...
        bytes32[] memory merkle_layer = new bytes32[](length / 276 + 1);
        result.unsettled_merkle_layer = new bytes32[](length /276 + 1);

        for (uint256 i = 32; i < length; i += 276) {
            leaf = getLockDataFromMerkleTree(merkle_tree_leaves, i);

            if (leaf.unsettled == true) {
                result.unsettled_merkle_layer[count] = leaf.lockhash;
                result.unsettled_amount += leaf.locked_amount;
                leaf.locked_amount = 0;
                count +=1;
            }
            result.unlocked_amount += leaf.locked_amount;
            merkle_layer[i / 276] = leaf.lockhash;
        }

        // merkle_root = getMerkleRoot(length, merkle_layer);
        // unsettled_merkle_root = getMerkleRoot(count * 276, unsettled_merkle_layer);

        result.merkle_root = getMerkleRoot(length, merkle_layer);
        if (result.unsettled_amount !=0) {
            result.unsettled_merkle_root = getMerkleRoot(count * 276, result.unsettled_merkle_layer);
        }
        return result;
    }

    function getMerkleRoot(uint256 length, bytes32[] memory merkle_layer) internal pure returns(bytes32){
        bytes32 lockhash;
        uint256 i;
        length /= 276;
        while (length > 1) {
    ...
    function getLockDataFromMerkleTree(bytes memory merkle_tree_leaves, uint256 offset)
        internal
        returns (leafData memory)
    {
        uint256 locked_amount;
        uint256 expiration;
        bytes32 secrethash;
        uint256 selected_incentive;
        uint256 selected_delay;
        uint256 aux_incentive;
        uint256 aux_delay;
        uint256 start_time;
        address target;
        ...
        if (merkle_tree_leaves.length <= offset) {
            // leaf.unsettled = false;
            // leaf.lockhash = lockhash;
            // leaf.locked_amount = 0;
            return leaf;
        }

        assembly {
            locked_amount := mload(add(merkle_tree_leaves, offset))
            expiration := mload(add(merkle_tree_leaves, add(offset, 32)))
            secrethash := mload(add(merkle_tree_leaves, add(offset, 64)))
            selected_incentive := mload(add(merkle_tree_leaves, add(offset, 96)))
            selected_delay := mload(add(merkle_tree_leaves, add(offset, 128)))
            aux_incentive := mload(add(merkle_tree_leaves, add(offset, 160)))
            aux_delay := mload(add(merkle_tree_leaves, add(offset, 192)))
            start_time := mload(add(merkle_tree_leaves, add(offset, 224)))
            target := mload(add(merkle_tree_leaves, add(20, 256)))
        }

        // Calculate the lockhash for computing the merkle root
        leaf.lockhash = keccak256(abi.encodePacked(locked_amount, expiration, secrethash, selected_incentive,
                                              selected_delay, aux_incentive, aux_delay, start_time, target));
        (leaf.unsettled, leaf.locked_amount) = calculateLockedAmount(locked_amount, expiration, secrethash, target, start_time,
                                                           [selected_incentive, aux_incentive], [selected_delay , aux_delay]);
    ...
    function calculateLockedAmount (
        ...
        uint256[2] memory incentive_bundle, uint256[2] memory delay_bundle)
        ...
        if (msg.sender == target) {
            // target dispute = start_time -> end_time
            end_time = secret_registry.setEndTime(secrethash, target, start_time);
        }
...
//             locked_amount = locked_amount;
            ...
            locked_amount = locked_amount - incentive_bundle[0] + final_incentive;
        }
